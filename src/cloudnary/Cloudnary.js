...
//*Configuration-->from documentation reading
cloudinary.config({ 
   ...
   export const uploadImageToCloudinary=async (localpath)=> {
        ...
        const uploadResult= await cloudinary.uploader.upload(localpath, {resource_type:"image" });
           console.log("image file uploaded",uploadResult.url);
    ...
    export const uploadVideoToCloudinary=async(localpath)=>{
        try{
            if(!localpath) return null;
            //* Upload an image
        const uploadResult= await cloudinary.uploader.upload(localpath, {resource_type:"video" });
           console.log("video file uploaded",uploadResult.url);
           fs.unlinkSync(localpath);
           return uploadResult;
        } catch(error){
           fs.unlinkSync(localpath);//remove locally saved file as the upload operation got failed
